...
平台作为合约 owner，确实是唯一能调用 `participate` 和 `drawWinner` 的角色。但它无法在不被发现的情况下作弊：

1. **无法增删参与者**：报名记录在截止时已全部上链，任何修改都是一笔公开交易，链上永久留痕
2. **无法操控 seed**：seed 由公开市场数据计算，平台不能自己编一个数，因为任何人都可以用当天收盘价重新计算并比对
3. **无法选择性开奖**：算法是确定性的，给定 seed 和参与名单，结果是唯一的。如果平台换一个 seed 重新开奖，参数变化同样留痕在链上
...
CREATE TABLE activities (
    ...
    status        ENUM('active','drawed','canceled') NOT NULL DEFAULT 'active',
...
- **seed 未存储在合约状态中**：目前需从交易 input data 获取，建议在 `drawWinner` 中 emit 事件携带 seed，降低验证门槛（Go 后端会将 seed 存入 `draw_results` 表来弥补）
- **`triggerDraw()` 为残留代码**（`MyLuckDraw.sol:183`）：该函数未完成且 `drawWinner` 已覆盖其逻辑，可清理
- **未编写测试**：`test/` 目录为空
- **`deploy.js` 未更新**：仍为 Hardhat 初始模板，部署需参考 `run.js` 的逻辑
