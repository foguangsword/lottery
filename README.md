...
| 阶段 | 平台的不公平行为 | 能否被链上防止 |
|------|----------|---------------|
| **报名阶段** | 可以拒绝某些用户报名（在链下拦截） | ❌ 不能防止。用户无法绕过前端直接调用合约 |
| **报名阶段** | 可以用自己生成的地址报名，增加一定的中奖概率 | ❌ 不能防止。任何"无门槛"架构都难以根除 Sybil 攻击 |
| **报名截止后** | 增删或修改参与者名单 | ✅ 能防止。名单已上链，不可篡改 |
| **开奖阶段** | 伪造 seed | ✅ 能防止。seed 来自公开股市数据，任何人可独立验算 |
| **开奖阶段** | 篡改中奖算法或结果 | ✅ 能防止。算法开源且确定性执行，链上留痕 |
